...
### The reason why we cannot use return function is because return greet.strip() in correct_greet only checks the exact match / answer which is only {"hello"} ###
...
### That is why we need to use .startswith() function so that nything that starts with "hello" or "h" will be acceptable and otherwise false ###
...
### Conclusion ###
# 1. Use return function only if the answer in the case/task are fixed answers/data. return function is used for true or false scenario/case. return function is used in strict case (it can only acccept fixed answers/data only).
# 2. Use .startswith() function if the correct answer for the case/task begins with a certain letters or words (e.g., "hello" or "hello there" or "h"), the answers will be acceptable instead of requiring the entire answer to be an exact match. If it requires to be an exact match, then use return function.
# 3. .startswith() function is more lenient.